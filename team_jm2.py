...
def load_twitter_sentiment_model():
    return joblib.load(open("Trained_models/sgdc_model.pkl", 'rb'))
...
# Twitter Sentiment page
    ...
    sgdc_model = joblib.load("Trained_models/sgdc_model.pkl")
    svm_model = joblib.load("Trained_models/svm_model.pkl")

    # Model selection
    model_options = ["Ridge", "SGDC", "SVM"]
    ...
    oneh.fit(raw[features])
    X_input = oneh.transform(raw[features])
...
if st.button("Classify"):
        ...
        if not tweet_text:
            ...
            y_train = raw['sentiment']

            # Assuming 'features' is the list of features you want to one-hot encode
            oneh = OneHotEncoder(handle_unknown="ignore")
            oneh.fit(raw[features])

            # Transform input text with the vectorizer
            vect_text = tweet_cv.transform([tweet_text])

            # One-hot encode the input text
            X_input_text = oneh.transform(raw[features])

            # Load the selected model based on user choice
            if selected_model == "Ridge":
                # Train the Ridge model
                ridge_model = RidgeClassifier()
                ridge_model.fit(X_input, y_train)

                # Make predictions using the trained model
                prediction = ridge_model.predict(X_input_text)

            elif selected_model == "SGDC":
                # Train the SGDC model
                sgdc_model = SGDClassifier()
                sgdc_model.fit(X_input, y_train)

                # Make predictions using the trained model
                prediction = sgdc_model.predict(X_input_text)

            elif selected_model == "SVM":
                # Train the SVM model
                svm_model = SVC()
                svm_model.fit(X_input, y_train)

                # Make predictions using the trained model
                prediction = svm_model.predict(X_input_text)
...
# Model Comparison page
    ...
    model_names = ['Ridge Model', 'SVM', 'SDG Classifier']
    accuracy = [0.7162, 0.7408, 0.6922]
    precision_positive = [0.78, 0.89, 0.85]
    precision_negative = [0.75, 0.88, 0.84]
    recall_positive = [0.92, 0.94, 0.89]
    recall_negative = [0.85, 0.91, 0.88]
    f1_score_positive = [0.84, 0.91, 0.87]
    f1_score_negative = [0.79, 0.90, 0.86]
    ...
    def display_ridge_model_information():
        ...
        st.write("71.62% Accuracy")
        st.write("Strengths: Good at predicting positive sentiment.")
    ...
    def display_sdg_classifier_information():
        st.header("SDG Classifier Information")
        st.write("Accuracy: 69.22%")
        st.write("Strengths: Competitive overall performance.")
        st.write("Weaknesses: Faces challenges in predicting negative and neutral sentiments. Visual Performance")
    ...
    elif selected_model == 'SDG Classifier':
        display_sdg_classifier_information()
